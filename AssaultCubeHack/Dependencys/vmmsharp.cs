...
namespace Framework
{
    ...
    public sealed class Vmm : IDisposable
    {
        ...
        private static unsafe IntPtr Initialize(out LeechCore.LC_CONFIG_ERRORINFO ConfigErrorInfo, params string[] args)
        {
            IntPtr pLcErrorInfo;
            int cbERROR_INFO = Marshal.SizeOf<lci.LC_CONFIG_ERRORINFO>();
            IntPtr hVMM = VMMDLL_InitializeEx(args.Length, args, out pLcErrorInfo);
        ...
        {
            IntPtr hS = VMMDLL_Scatter_Initialize(hVMM, pid, flags);
        ...
        public unsafe PROCESS_INFORMATION ProcessGetInformation(uint pid)
        {
            ...
            {
                Marshal.WriteInt64(new IntPtr(pb + 0), unchecked((long)VMMDLL_PROCESS_INFORMATION_MAGIC));
                Marshal.WriteInt16(new IntPtr(pb + 8), unchecked((short)VMMDLL_PROCESS_INFORMATION_VERSION));
                result = VMMDLL_ProcessGetInformation(hVMM, pid, pb, ref cbENTRY);
                if (!result) { return new PROCESS_INFORMATION(); }
                VMMDLL_PROCESS_INFORMATION n = Marshal.PtrToStructure<VMMDLL_PROCESS_INFORMATION>((IntPtr)pb);
        ...
        public unsafe IMAGE_DATA_DIRECTORY[] ProcessGetDirectories(uint pid, string wszModule)
        {
            ...
            fixed (byte* pb = new byte[16 * cbENTRY])
            {
                ...
                for (int i = 0; i < 16; i++)
                {
                    VMMDLL_IMAGE_DATA_DIRECTORY n = Marshal.PtrToStructure<VMMDLL_IMAGE_DATA_DIRECTORY>((IntPtr)(pb + i * cbENTRY));
        ...
        public unsafe IMAGE_SECTION_HEADER[] ProcessGetSections(uint pid, string wszModule)
        {
            ...
            fixed (byte* pb = new byte[cData * cbENTRY])
            {
                ...
                {
                    VMMDLL_IMAGE_SECTION_HEADER n = Marshal.PtrToStructure<VMMDLL_IMAGE_SECTION_HEADER>((IntPtr)(pb + i * cbENTRY));
        ...
        public unsafe MAP_PTEENTRY[] Map_GetPte(uint pid, bool fIdentifyModules = true)
        {
            ...
            {
                VMMDLL_MAP_PTEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_PTEENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_PTEENTRY e;
                e.vaBase = n.vaBase;
        ...
        public unsafe MAP_VADENTRY[] Map_GetVad(uint pid, bool fIdentifyModules = true)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_VADENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_VADENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_VADENTRY e;
                e.vaStart = n.vaStart;
                e.vaEnd = n.vaEnd;
        ...
        public unsafe MAP_VADEXENTRY[] Map_GetVadEx(uint pid, uint oPages, uint cPages)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_VADEXENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_VADEXENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_VADEXENTRY e;
                e.tp = n.tp;
                e.iPML = n.iPML;
        ...
        public unsafe MAP_MODULEENTRY[] Map_GetModule(uint pid, bool fExtendedInfo)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_MODULEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_MODULEENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_MODULEENTRY e;
        ...
        public unsafe MAP_UNLOADEDMODULEENTRY[] Map_GetUnloadedModule(uint pid)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_UNLOADEDMODULEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_UNLOADEDMODULEENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
        ...
        public unsafe MAP_EATENTRY[] Map_GetEAT(uint pid, string wszModule, out MAP_EATINFO EatInfo)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_EATENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_EATENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_EATENTRY e;
        ...
        public unsafe MAP_IATENTRY[] Map_GetIAT(uint pid, string wszModule)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_IATENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_IATENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_IATENTRY e;
                e.vaFunction = n.vaFunction;
                e.wszFunction = n.wszFunction;
        ...
        public unsafe MAP_HEAP Map_GetHeap(uint pid)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_HEAPENTRY nH = Marshal.PtrToStructure<VMMDLL_MAP_HEAPENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_HEAPSEGMENTENTRY nH = Marshal.PtrToStructure<VMMDLL_MAP_HEAPSEGMENTENTRY>((IntPtr)(nM.pSegments.ToInt64() + i * cbSEGENTRY));
        ...
        public unsafe MAP_HEAPALLOCENTRY[] Map_GetHeapAlloc(uint pid, ulong vaHeapOrHeapNum)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_HEAPALLOCENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_HEAPALLOCENTRY>((IntPtr)(pHeapAllocMap.ToInt64() + cbMAP + i * cbENTRY));
                m[i].va = n.va;
                m[i].cb = n.cb;
                m[i].tp = n.tp;
            }
        ...
        public unsafe MAP_THREADENTRY[] Map_GetThread(uint pid)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_THREADENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_THREADENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_THREADENTRY e;
                e.dwTID = n.dwTID;
                e.dwPID = n.dwPID;
        ...
        public unsafe MAP_HANDLEENTRY[] Map_GetHandle(uint pid)
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_HANDLEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_HANDLEENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_HANDLEENTRY e;
        ...
        public unsafe MAP_NETENTRY[] Map_GetNet()
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_NETENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_NETENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_NETENTRY e;
                e.dwPID = n.dwPID;
                e.dwState = n.dwState;
                e.dwPoolTag = n.dwPoolTag;
        ...
        public unsafe MAP_PHYSMEMENTRY[] Map_GetPhysMem()
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_PHYSMEMENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_PHYSMEMENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_PHYSMEMENTRY e;
                e.pa = n.pa;
                e.cb = n.cb;
                m[i] = e;
        ...
        public unsafe MAP_POOLENTRY[] Map_GetPool()
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_POOLENTRY nE = Marshal.PtrToStructure<VMMDLL_MAP_POOLENTRY>((IntPtr)(pN.ToInt64() + cbMAP + i * cbENTRY));
                eM[i].va = nE.va;
                eM[i].cb = nE.cb;
        ...
        public unsafe MAP_USERENTRY[] Map_GetUsers()
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_USERENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_USERENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_USERENTRY e;
                e.szSID = n.wszSID;
                e.wszText = n.wszText;
        ...
        public unsafe MAP_SERVICEENTRY[] Map_GetServices()
        {
            ...
            for (int i = 0; i < nM.cMap; i++)
            {
                VMMDLL_MAP_SERVICEENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_SERVICEENTRY>((IntPtr)(pMap.ToInt64() + cbMAP + i * cbENTRY));
                MAP_SERVICEENTRY e;
                e.vaObj = n.vaObj;
                e.dwPID = n.dwPID;
                e.dwOrdinal = n.dwOrdinal;
        ...
        public unsafe MAP_PFNENTRY[] Map_GetPfn(params uint[] pfns)
        {
            ...
            fixed (byte* pbPfns = dataPfns)
            {
                ...
                fixed (byte* pb = new byte[cbPfns])
                {
                    ...
                    VMMDLL_MAP_PFN pm = Marshal.PtrToStructure<VMMDLL_MAP_PFN>((IntPtr)pb);
                    ...
                    {
                        VMMDLL_MAP_PFNENTRY n = Marshal.PtrToStructure<VMMDLL_MAP_PFNENTRY>((IntPtr)(pb + cbMAP + i * cbENTRY));
        ...
        public unsafe REGISTRY_HIVE_INFORMATION[] RegHiveList()
        {
            ...
            fixed (byte* pb = new byte[cHives * cbENTRY])
            {
                ...
                {
                    VMMDLL_REGISTRY_HIVE_INFORMATION n = Marshal.PtrToStructure<VMMDLL_REGISTRY_HIVE_INFORMATION>((IntPtr)(pb + i * cbENTRY));
